    ...
    <main>
        <section class="card">
            ...
            <form method="POST" action="{{ $action }}" enctype="multipart/form-data">
                ...
                <label>
                    ชื่อเกม
                    <input name="name" value="{{ old('name', $game->name) }}" required autofocus>
                ...
                <div class="grid">
                    <label>
                        Slug
                        <input name="slug" value="{{ old('slug', $game->slug) }}" placeholder="เว้นว่างให้ระบบสร้างให้">
                        @error('slug') <span class="error">{{ $message }}</span> @enderror
                    </label>

                    <label>
                        ผู้ให้บริการ / Publisher
                        <input name="publisher" value="{{ old('publisher', $game->publisher) }}">
                ...
                <label>
                    ...
                    @if ($game->imageUrl())
                        <span class="hint">รูปปัจจุบัน: {{ $game->image_path }}</span>
                    @endif
                ...
                <label>
                    Path รูปเกม
                    <input name="image_path" value="{{ old('image_path', $game->image_path) }}" placeholder="/figma/game-mobile-legends.webp">
                ...
                <div class="grid">
                    <label>
                        ...
                        <select name="status" required>
                            @foreach ($statusLabels as $value => $label)
                                <option value="{{ $value }}" @selected(old('status', $game->status) === $value)>{{ $label }}</option>
                            @endforeach
                        </select>
                    ...
                    <label>
                        ลำดับแสดงผล
                        <input name="sort_order" type="number" min="0" value="{{ old('sort_order', $game->sort_order ?? 0) }}">
                ...
                <div class="actions">
                    ...
                    <a class="button secondary" href="{{ route('admin.products.index') }}">กลับ</a>
                </div>
            </form>
        </section>
    </main>
