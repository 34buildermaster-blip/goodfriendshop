...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; }
        ...
        h2 { margin: 24px 0 0; font-size: 20px; }
        ...
        input, select, textarea { width: 100%; border: 1px solid var(--line); border-radius: 16px; background: rgba(255,255,255,.04); color: #fff; font: inherit; outline: none; }
...
<body>
    ...
    <main>
        <section class="card">
            ...
            <p class="lead">ตรวจข้อมูลลูกค้า อัปเดตสถานะงาน และบันทึกข้อมูลการชำระเงิน</p>
            ...
            <div class="grid">
                ...
                <div class="info"><span>ข้อมูลบัญชี</span><strong>{{ $order->player_identifier }}</strong><p class="muted">Server: {{ $order->server_identifier ?: '-' }}</p></div>
            ...
            @if (! empty($order->extra_fields))
                <div class="info" style="margin-top: 14px;">
                    <span>ข้อมูลเพิ่มเติม</span>
                    <pre class="muted" style="white-space: pre-wrap; margin: 10px 0 0;">{{ json_encode($order->extra_fields, JSON_PRETTY_PRINT | JSON_UNESCAPED_UNICODE) }}</pre>
                </div>
            @endif
            ...
            <form method="POST" action="{{ route('admin.orders.update', $order) }}">
                ...
                <div class="grid">
                    <label>
                        ช่องทางชำระเงิน
                        <input name="payment_method" value="{{ old('payment_method', $order->payment_method) }}" placeholder="Wepay / PromptPay / Bank transfer">
                        @error('payment_method') <span class="error">{{ $message }}</span> @enderror
                    </label>
                    <label>
                        Reference / Transaction ID
                        <input name="payment_reference" value="{{ old('payment_reference', $order->payment_reference) }}">
                        @error('payment_reference') <span class="error">{{ $message }}</span> @enderror
                    </label>
                    <label>
                        เวลาที่ชำระเงิน
                        <input name="paid_at" type="datetime-local" value="{{ old('paid_at', $order->paid_at?->format('Y-m-d\TH:i')) }}">
                ...
                <label>
                    โน้ตการชำระเงิน
                    <textarea name="payment_note">{{ old('payment_note', $order->payment_note) }}</textarea>
